...
class StudentList():
    ...
    def print_student_list(self):
        for student in self.StudentList:
            print(f'Course for {student.FirstName} {student/LastName} {T-Number: {student.TNumber}})')
            for course in student.CourseList:
                print (f' {course.department}{course.number} - {course.name}')
            print()
        return
    
    def add_student_from_file(slef, filename):
...
class CourseList:
    ...
    def add_course_from_file(self, filename):
        ...
        for course in courses:
            deparment, number, name, room, meetingtimes = course.split(',')
            self.add_course(deparment, number, name, room, meetingtimes)
        return
...
for reg in registration_data:
    ...
    course_index = mycourselist.find_course(deparment, number)
    mycourse = mycourselist.Courselist[course.index]
